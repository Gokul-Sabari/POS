...
// const PRODUCT_API_URL = 'https://erpsmt.in/api/pos/productMaster';
...
const fetchProducts = useCallback(async () => {
  ...
  try {
    ...
    if (posProductData && Array.isArray(posProductData) && posProductData.length > 0) {
      const productEntries = posProductData[0].Product_Entry || [];
     
      const cleanedProducts = productEntries
        .filter(
          (p) => (
            // Number(p?.Bal_Qty) || 0) > 0 &&
             p?.Is_Active == 1)
        )
        .map((p) => ({
          id: p.Product_Id,
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            currentProducts = currentProducts.filter(product => 
                ...
                product.printName.toLowerCase().includes(query)
            );
        }
    ...
    if (isLoading) {
        return (
            <div className="w-[60%] flex-1 p-4 flex items-center justify-center bg-white rounded-lg shadow-xl">
                <p className="text-xl text-teal-700 font-semibold">Loading Product Master... (API Call)</p>
            </div>
        );
    }
            ...
            return (
                ...
                >
                    <div>
                        ...
                        <p className="text-xs text-gray-600 truncate">{product.printName}</p>
                    </div>
                    
                    <div className="mt-1">
                        <p className="text-teal-700 font-extrabold text-xl">Rs {product.price.toFixed(2)}</p>
                        <p className={`text-xs font-semibold ${product.balanceQty > 0 ? 'text-green-600' : 'text-red-500'}`}>
                            {product.balanceQty > 0 
